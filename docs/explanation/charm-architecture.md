...
<!-- Example text
At its core, the falco charm is <software> that does <brief description>.
...
As a result, if you run a `kubectl get pods` on a namespace named for the Juju model you've deployed the falco charm into, you'll see something like the following:

```bash
NAME                             READY   STATUS    RESTARTS   AGE
falco-0                   N/N     Running   0         6h4m
```

This shows there are <NUMBER> containers - <describe what the containers are>.
-->

## High-level overview of falco deployment

The following diagram shows a typical deployment of the falco charm.
<!-- 
...
The following diagram shows the architecture of the falco charm:
...
We use [Rockcraft](https://canonical-rockcraft.readthedocs-hosted.com/en/latest/) to build OCI Images for falco.
The images are defined in [falco rock](link to rock).
